...
function upgradeExtensionHostHelperRuntime(text, context) {
  ...
  let next = text.replaceAll(EXTENSION_VSCODE_ALIAS, 'require("vscode")').replace(oldGuard, newGuard);
...
function patchHeader(text, context, file) {
  ...
  next = patchHeaderRefreshHook(next, context);
  if (next.includes('codexLocalGroupsHeaderPatchVersion=38') && next.includes('action:`getMetadata`') && next.includes('dispatchHostMessage({type:`new-chat`})') && next.includes('metadata-actions-') && next.includes('r||s') && next.includes('t<1e12?t*1e3:t')) {
    return next;
  }
  return patchHeaderGroupHelper(next, context);
}
...
function patchHeaderRefreshHook(text, context) {
  ...
  next = next.replace(/codexLocalGroupsHeaderPatchVersion=(?:1[789]|2[0-9]|3[0-7])/g, 'codexLocalGroupsHeaderPatchVersion=38');
  return next;
}
...
function safeHeaderHelper(metadata, messenger, kindFnName) {
  return patchHeaderPendingItems(headerHelper(metadata, messenger, kindFnName))
    .replace('var codexLocalGroupsHeaderPatchVersion=38;', 'var codexLocalGroupsHeaderSafePatchVersion=3;');
}
