...
function patchExtensionMetadataHelper(text, context) {
  if (text.includes('codexLocalGroupsPatchVersion=12')) {
    return text;
  }
...
function patchHeader(text, context, file) {
  ...
  if (next.includes('codexLocalGroupsHeaderPatchVersion=24') && next.includes('codexLocalGroupsNormalizeGroupName')) {
    return next;
  }
...
function patchHeaderRefreshHook(text, context) {
  ...
  next = next.replace(/codexLocalGroupsHeaderPatchVersion=1[789]|codexLocalGroupsHeaderPatchVersion=2[1234]/g, 'codexLocalGroupsHeaderPatchVersion=24');
  return next;
}
...
function upgradeHeaderHelperRuntime(text) {
  ...
  if (!next.includes('function codexLocalGroupsSetBusy')) next = next.replace(/function codexLocalGroupsPromptTitle/, 'function codexLocalGroupsSetBusy(e,t){try{let n=String(e.currentTarget.textContent||\`\`);e.currentTarget.textContent=t,setTimeout(()=>{e.currentTarget&&(e.currentTarget.textContent=n)},1200)}catch{}}function codexLocalGroupsPromptTitle');
  ...
  next = next.replace(/onClose:n\},o\.key\)/g, 'onClose:()=>{b.dispatchHostMessage({type:`navigate-to-route`,path:`/local/${o.conversation.id}`}),n()},o.key)');
  return next;
}
