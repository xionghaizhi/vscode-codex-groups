...
function patchExtensionThreadList(text, originalText, context) {
  ...
  if (text.includes(extensionThreadListWithoutCwds())) {
  ...
  return replaceRegexOnce(text, latest, () => next, context, 'extension cwd-filtered thread list latest');
}

function extensionThreadListNew(text) {
  const vscodeName = symbolBefore(text, 'onDidChangeChatSessionItemsEmitter=new ', '.EventEmitter;') || 'wl';
  const requestProvider = symbolBefore(text, 'this.codexAppServer.sendRequest(', ',r,"thread/list"') || '_le';
  const modelProvider = symbolBefore(text, 'modelProviders:e?[', ']:null') || 'HS';
  const sourceKinds = symbolBefore(text, 'archived:!1,sourceKinds:', '})') || 'Yf';
...
function patchHeaderGroupHelper(text, context) {
  ...
  const upgradedV3 = replaceToMarker(text, currentStartV3, 'var ot=', headerHelper(context.metadata, messenger, 'at'));
  if (upgradedV3) {
    return upgradedV3;
  }
  const previousV3 = replaceToMarker(text, previousStartV3, 'var ot=', headerHelper(context.metadata, messenger, 'at'));
  if (previousV3) {
    return previousV3;
  }
  ...
  if (text.includes('function at(e){return e.kind===`remote`}var ot=')) {
    return replaceOnce(text, 'function at(e){return e.kind===`remote`}var ot=', `${headerHelper(context.metadata, messenger, 'at')}var ot=`, context, 'header local groups helper latest');
  }
