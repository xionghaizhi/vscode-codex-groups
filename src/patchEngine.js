...
function patchHeader(text, context, file) {
  ...
  if (next.includes('codexLocalGroupsHeaderPatchVersion=31') && next.includes('codex-local-groups-inline-actions absolute')) {
    return next;
  }
...
function patchHeaderRefreshHook(text, context) {
  ...
  next = next.replace(/codexLocalGroupsHeaderPatchVersion=(?:1[789]|2[0-9]|30)/g, 'codexLocalGroupsHeaderPatchVersion=31');
  return next;
}
