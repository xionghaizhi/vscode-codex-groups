...
function patchExtensionThreadList(text, context) {
  ...
  const withCwds = extensionThreadListWithCwdsRegex();
  if (withCwds.test(text)) {
    return replaceRegexOnce(text, extensionThreadListWithCwdsRegex(), () => next, context, 'extension remove server cwd filter');
  }
  if (text.includes('async requestAllThreadList(e)')) {
    return text;
  }
...
function extensionThreadListNew(text) {
  ...
  return `async requestAllThreadList(e){let r=[],n=null;do{let o=await this.requestThreadList(e,n);r.push(...o.data),n=o.nextCursor??null}while(n);return{data:r}}requestThreadList(e,r){let n=String(this.nextRequestId++),o=new Promise((i,s)=>{this.requestToCallback.set(n,a=>{if(a.error){s(new Error(a.error.message));return}if(a.result==null){s(new Error("No result in response"));return}i(a.result)})}),c={limit:200,cursor:r,sortKey:"created_at",modelProviders:e?[${modelProvider}]:null,archived:!1,sourceKinds:${sourceKinds}};return this.codexAppServer.sendRequest(${requestProvider},n,"thread/list",c),o}`;
}

function extensionThreadListWithCwdsRegex() {
  return /async requestAllThreadList\(e\)\{let r=\[\],n=null;do\{let o=await this\.requestThreadList\(e,n\);r\.push\(\.\.\.o\.data\),n=o\.nextCursor\?\?null\}while\(n\);return\{data:r\}\}requestThreadList\(e,r\)\{let n=String\(this\.nextRequestId\+\+\),o=new Promise\(\(i,s\)=>\{this\.requestToCallback\.set\(n,a=>\{if\(a\.error\)\{s\(new Error\(a\.error\.message\)\);return\}if\(a\.result==null\)\{s\(new Error\("No result in response"\)\);return\}i\(a\.result\)\}\)\}\),s=[A-Za-z_$][\w$]*\.workspace\.workspaceFolders\?\.map\(a=>a\.uri\.fsPath\)\.filter\(Boolean\)\?\?\[\],c=\{limit:200,cursor:r,sortKey:"created_at",modelProviders:e\?\[[A-Za-z_$][\w$]*\]:null,archived:!1,sourceKinds:[A-Za-z_$][\w$]*\};s\.length>0&&\(c\.cwds=s\);return this\.codexAppServer\.sendRequest\([A-Za-z_$][\w$]*,n,"thread\/list",c\),o\}/;
}
...
function patchAppServerManagerSignals(text, context) {
  let next = text.replace(/var codexLocalGroupsRecentInitialMeta=[\s\S]*?;var codexLocalGroupsRecentPatchVersion=2;/, 'var codexLocalGroupsRecentPatchVersion=2;');
  ...
  if (next.includes('codexLocalGroupsRecentPatchVersion=1')) {
    next = next.replace('codexLocalGroupsRecentPatchVersion=1', 'codexLocalGroupsRecentPatchVersion=2');
    next = next.replace(/function codexLocalGroupsRecentThreadListParams\(e\)\{let t=codexLocalGroupsRecentProjectRoots\(\);return t\.length\?\(e=\{\.\.\.e,cwds:t\},typeof e\.limit===`number`&&e\.limit<200&&\(e\.limit=200\),e\):e\}/g, 'function codexLocalGroupsRecentThreadListParams(e){return typeof e.limit===`number`&&e.limit<200?{...e,limit:200}:e}');
  }
  if (!next.includes('codexLocalGroupsRecentPatchVersion=2')) {
  ...
  next = next.replace(allNewV2, allOldV2).replace(allNewV1, allOldV1);
