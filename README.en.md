...
<p align="center">
  ...
  <img alt="release" src="https://img.shields.io/badge/release-v0.0.16-blue">
...
```bash
cp -r . ~/.vscode/extensions/vscode-codex-groups-0.0.16
```

For Remote VSCode Server, copy it into the remote extensions directory, for example:

```bash
cp -r . ~/.vscode-server/extensions/vscode-codex-groups-0.0.16
```
...
```bash
code --install-extension vscode-codex-groups-0.0.16.vsix
```
...
```bash
cd ~/.vscode-server/extensions/vscode-codex-groups-0.0.16
