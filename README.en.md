...
- If Codex `26.5730.61639` shows “could not start / couldn't load its resources,” upgrade to v0.0.54, run `Apply Patches`, then Reload Window. The fixed 30-second watchdog can preempt a healthy Remote Webview that needs 61-71 seconds; v0.0.54 extends it to 120 seconds only for this version and preserves the real failure fallback.
- If Codex `26.5803.41515` still takes more than 30 seconds to open, the current clean log shows a `34,566ms` route mount followed by ready about 20ms later. The 120-second patch only prevents a premature timeout; it neither forces a 120-second wait nor shortens upstream bundle loading and React route mounting.
...
- If the current project shows groups from other projects, upgrade to v0.0.54, run `Apply Patches`, then Reload Window. The project history is isolated by the current window's `activeWorkspaceRoot`, while child directories are merged into the root project.
- If one requirement group renders too many conversations at once, upgrade to v0.0.54 and Reload Window. Each group independently starts at five rows, `Show more` adds ten, and collapse controls return it to fifteen or five.
- If `Set title`, `Set group`, or `Start chat in this group` does nothing, upgrade to v0.0.54, run `Apply Patches`, then Reload Window. This version supports the Codex 26.727/26.5730/26.5803 messenger exports and Extension Host callback.
- If a resumed conversation has no terminal tools, upgrade to v0.0.54, run `Apply Patches`, then Reload Window. For `26.721.41059` with a resolvable custom provider, app-server falls back to HTTP POST without editing `config.toml`.
