...
<p align="center">
  ...
  <img alt="release" src="https://img.shields.io/badge/release-v0.0.60-blue">
...
- By default, patches only the metadata bridge, grouped recent-list rendering, an isolated Codex 26.721/26.727/26.5730/26.5803/26.5810/26.5814/26.5818 project-history query, and the fixed menu height; it does not expand the shared recent store or write conversation data.
- Migration from:
...
- On Codex 26.721/26.727/26.5730/26.5803/26.5810/26.5814/26.5818, it pages an isolated project-history query and strictly keeps the active `activeWorkspaceRoot` plus descendants. While the workspace root is loading, it fails closed with an empty list instead of showing another project.
...
- On Codex 26.721/26.727/26.5730/26.5803/26.5810/26.5814/26.5818, the recent menu gets an actual `600px` height, remains clamped by Radix on short windows, and scrolls inside the list region. Other versions keep their original height; React compiler cache state, authentication, plugins, and network requests remain unchanged.
- On Codex 26.721/26.727/26.5730/26.5803/26.5810/26.5814/26.5818 it preserves or enables the native subagent activity panel and keeps `Max` and `Ultra` in the local 5.6 Sol reasoning selector.
...
```bash
cp -r . ~/.vscode/extensions/vscode-codex-groups-0.0.60
```

For Remote VSCode Server, copy it into the remote extensions directory, for example:

```bash
cp -r . ~/.vscode-server/extensions/vscode-codex-groups-0.0.60
```
...
```bash
code --install-extension vscode-codex-groups-0.0.60.vsix
```
...
```bash
cd ~/.vscode-server/extensions/vscode-codex-groups-0.0.60
...
- If Codex `26.5730.61639` shows “could not start / couldn't load its resources,” upgrade to v0.0.60, run `Apply Patches`, then Reload Window. The fixed 30-second watchdog can preempt a healthy Remote Webview that needs 61-71 seconds; v0.0.60 extends it to 120 seconds only for this version and preserves the real failure fallback.
- If Codex `26.5810.41047` / `26.5810.52044` shows “Codex could not start / The extension could not start its user interface,” upgrade to v0.0.60, run `Apply Patches`, then Reload Window. The watchdog is now `jP`; the old `onTimeout()},3e4` anchor is gone. v0.0.60 rewrites the unique `timeoutMs:3e4},3e4` pair to 120 seconds.
- If Codex `26.5814.41407` loses groups, local titles, or project history after upgrading, install v0.0.60, run `Apply Patches`, then Reload Window. This build uses the `YI` watchdog and `Q9` message parser; the old 5810 minified anchors cannot be reused.
- If Codex `26.5818.31338` loses groups, titles, Sol reasoning levels, project history, or the subagent panel chain after upgrading, install v0.0.60, run `Apply Patches`, then Reload Window. This build uses the `QP` watchdog and `nY` message parser; the real Header messenger/execution-target exports are `Flt`/`c0`, while the old `Vst`/`U$` names now point to unrelated exports.
...
- If the current project shows groups from other projects, upgrade to v0.0.60, run `Apply Patches`, then Reload Window. The project history is isolated by the current window's `activeWorkspaceRoot`, while child directories are merged into the root project.
- If one requirement group renders too many conversations at once, upgrade to v0.0.60 and Reload Window. Each group independently starts at five rows, `Show more` adds ten, and collapse controls return it to fifteen or five.
- If `Set title`, `Set group`, or `Start chat in this group` does nothing, upgrade to v0.0.60, run `Apply Patches`, then Reload Window. This version supports the Codex 26.727/26.5730/26.5803/26.5810/26.5814/26.5818 messenger exports and Extension Host callback.
- If a resumed conversation has no terminal tools, upgrade to v0.0.60, run `Apply Patches`, then Reload Window. For `26.721.41059` with a resolvable custom provider, app-server falls back to HTTP POST without editing `config.toml`.
