...
function configure265803Features(target, version = '26.5803.41515', appMainText = split265803AppMainText) {
  const assets = path.dirname(target.headerPath);
  ...
  fs.writeFileSync(target.appMainPath, appMainText);
  fs.writeFileSync(target.appServerManagerSignalsPath, split265803AppServerText);
  fs.writeFileSync(path.join(assets, 'app-initial-main265803.js'), appMainText);
...
const header26581041047Text = [
  ...
  'function xn(e){let t=(0,En.c)(34),{cloudtasksQuery:n,localConversations:r,onClose:i,autoFocusSearch:a,showFilters:o}=e,s=a!==void 0&&a,c=o===void 0||o,l=g(),u=Ce(),{authMethod:d}=m(),T=e=>!0,w=null,y=null;let E=r.filter(T),D=hn(n.data,r,w),F=D,B=F.map(e=>(0,Z.jsx)(An,{item:e,isActive:e.kind===`local`&&e.conversation!=null&&y===e.conversation.id,onClose:i,onActiveArchiveStart:u},e.key)),V=(0,Z.jsxs)(z.Section,{className:`vertical-scroll-fade-mask flex max-h-[60vh] flex-col gap-0 overflow-y-auto pb-1`,children:[B]});return(0,Z.jsxs)(`div`,{className:`flex max-h-[300px] w-[calc(var(--radix-popper-available-width)_-_var(--padding-panel))] flex-col gap-1`,children:[V]})}',
  'var messages={recentTasksMenu:1,search:{defaultMessage:`Search recent chats`}};function Sn(e){return e.kind===`remote`}function Cn(){}',
  'var An=(0,Dn.memo)(function(e){let t=(0,En.c)(24),{item:n,isActive:r,onClose:i,onActiveArchiveStart:a}=e;switch(n.kind){case`local`:{let e=null,c;return t[17]!==r||t[18]!==n.conversation.hostId||t[19]!==n.conversation.id||t[20]!==a||t[21]!==i||t[22]!==e?(c=(0,Z.jsx)(pt,{conversationId:n.conversation.id,hostId:n.conversation.hostId,isActive:r,metaContent:e,onClick:i,onActiveArchiveStart:a}),t[17]=r,t[18]=n.conversation.hostId,t[19]=n.conversation.id,t[20]=a,t[21]=i,t[22]=e,t[23]=c):c=t[23],c}}});',
  'const nativeMenu=(0,Q.jsx)(m,{contentClassName:`!pb-0 mt-[9px]`,triggerButton:oe,open:g,onOpenChange:_,children:ce});',
  'function Bn(e){let t=(0,Gn.c)(64),{allowInitialRouteBack:r,className:i,centerContent:a,desktopDeepLinkConversationId:s,title:c,onBack:l,trailing:u}=e,d=r!==void 0&&r;return c}function Vn(){}',
].join('');
const split26581041047AppMainText = [
  'class Messenger{static getInstance(){return new Messenger}dispatchMessage(){}dispatchHostMessage(){}}',
  'var Zl=Messenger.getInstance();function useMessenger(){Zl.dispatchMessage(`native`,{}),Zl.dispatchHostMessage({type:`native`})}',
  'function ZC(e){return{activeWorkspaceRoot:`/project`,isActiveWorkspaceRootLoading:!1}}',
  'function iBe({userSavedModelString:e,userSavedReasoningEffort:t,listModelsData:n}){let r=n?.models?.find(n=>n.model===e),i=r?.supportedReasoningEfforts?.map(e=>e.reasoningEffort),a=t!=null&&i!=null&&i.includes(t)?t:r?.defaultReasoningEffort;return{model:r?.model,reasoningEffort:a}}',
  ...
  'function dyn(e,t,n,r){let i=new Map;for(let[a,o]of e.entries())for(let e of r?.(t,o,a)??o.items){if(e.type===`subAgentActivity`){i.set(e.agentThreadId,{conversationId:e.agentThreadId,parentConversationId:t});continue}if(!(e.type!==`collabAgentToolCall`||e.tool!==`spawnAgent`))for(let r of e.receiverThreadIds)i.set(r,{conversationId:r,parentConversationId:t})}return Array.from(i.values())}',
  ...
  'function Dc(e,t){return t}var lJ=Dc($,(e,{get:t})=>{let n=t(lr,e);return uyn({cachedConversations:[],conversationTurns:n.turns,parentConversationId:e,sourceLinkedThreads:null}).filter(({conversationId:e})=>!t(blocked).includes(e))});',
  'function Pon(e,t){return e.flatMap((e,n)=>t?.some(t=>t.model===e.model&&t.supportedReasoningEfforts.some(({reasoningEffort:t})=>t===e.reasoningEffort))?[{...e,powerSettingIndex:n}]:[])}',
  'var Ion,Lon,Ron;Ion=[{id:`gpt-5.6-terra:low`,model:`gpt-5.6-terra`,modelLabel:`5.6 Terra`,reasoningEffort:`low`},{id:`gpt-5.6-sol:xhigh`,model:`gpt-5.6-sol`,modelLabel:`5.6 Sol`,reasoningEffort:`xhigh`}],Lon={id:`gpt-5.6-sol:ultra`,model:`gpt-5.6-sol`,modelLabel:`5.6 Sol`,reasoningEffort:`ultra`},Ron=[];',
  'function kon(e,{includeUltraInSlider:t=!1,removeXHigh:n=!1}={}){let r=Pon((t?[...Ion,Lon]:Ion).filter(({reasoningEffort:e})=>!n||e!==`xhigh`),e);if(r.length>=3)return r;let i=Pon(Ron.filter(({reasoningEffort:e})=>!n||e!==`xhigh`),e);return i.length>=3?i:[]}',
  'function wC(){return!0}var e4e=[];function u$(e,t){let n=e?.find(e=>e.model===t);return n==null?e4e.map(e=>({description:``,reasoningEffort:e})):n.supportedReasoningEfforts.filter(e=>wC(e.reasoningEffort))}',
  'function wc(e,t){return []}function jOr(){}function Ezn(){}function J6(){return{jsx(){}}}function dCn(e){return e}',
  'function DOr(e){let t=(0,jOr.c)(12),{activeConversationId:n,enabled:r,includeMentionItems:i}=e,a=wc(lJ,r?n:null),o,s;let parent=e=>e.parentConversationId===n,rw=a.filter(parent).filter(AOr);o=i?rw.map(kOr):[],s=rw.filter(OOr);let c=s,l=null;return{rows:a,visibleRows:c,mentionItems:o,firstApproval:l}}',
  'function OOr(e){return e.isCurrentParentTurn}function kOr(e){return{conversationId:e.conversationId,status:e.status}}function AOr(e){return e.canInteract&&e.displayName.trim().length>0}',
  'function xzn(e){let t=(0,Ezn.c)(40),{rows:n,agentCount:r,canStopAll:i,isStopAllDisabled:a,onOpenThread:o,onStopAll:s}=e;return{id:`composer.backgroundSubagents.summary`,rows:n,agentCount:r}}',
  'function aNr(){let ie=`x`,Ke=!0,Je={ui:{active:!0}},{rows:Ye,visibleRows:Xe}=DOr({activeConversationId:ie,enabled:Ke,includeMentionItems:Je.ui?.active===!0}),Ot=0,kt=!1,it=!1,fn=(Xe.length>0||kt)&&!it;dCn({subagentsPanel:fn});return fn?(0,J6.jsx)(xzn,{agentCount:Math.max(Xe.length,Ot),rows:Xe}):null}',
  'var app=`conversation.title untitledThreadLabel supportedReasoningEfforts defaultReasoningEffort networkConfig:{api:j} sdkExceptionUrl: safeGet makeRequest OAI-Language title:t(Bi,e) turns:t(Ote,e)`;',
  'export{Zl as Oat,ZC as KZ,lJ as FT};',
].join('');
const split26581041047AppServerText = [
  ...
  'function Ron(){return Bon(`recent-conversations`)}function Bon(e){return{data:[]}}',
  'function IF(e){return{createdAt:e.createdAt??0,updatedAt:e.updatedAt??0,recencyAt:e.recencyAt??null}}',
  'function Lk(e){return{id:e.conversationId??e.id,cwd:e.cwd}}',
  'function zF(){return!0}function BF(e){return e}function pRt(e){return e.name??null}',
  'function eH(){return{getDefault:()=>({getHostId:()=>`local`}),getForHostId:()=>null}}',
  'function YO(e){return e}function Oon(e){return()=>{for(let t of e)t()}}',
  'function kon({appServerRegistry:e,onStoreChange:t,subscribeToManager:n}){return()=>{}}',
  'var nH={useEffect(){}};var recentConversationsSortKey=1,threadList=`thread/list`;',
].join('');
const header26581052044Text = [
  'import{qZ as x,kat as m}from"./app-initial-main265810.js";',
  'function Mn(e){let s=a===void 0||a,l=r(),{authMethod:u}=p(),d=n(),f=ee(Ln),m=ee(Rn),{data:h}=te(),g=Ae(),z=null;return h}',
  'function xn(e){let t=(0,En.c)(34),{cloudtasksQuery:n,localConversations:i,onClose:a,autoFocusSearch:o,showFilters:s}=e,c=o!==void 0&&o,l=s===void 0||s,u=r(),f=we(),{authMethod:m}=p(),w=e=>!0,ee=null,y=null;let T=i.filter(w),E=hn(n.data,i,ee),F=E,B=F.map(e=>(0,Z.jsx)(An,{item:e,isActive:e.kind===`local`&&e.conversation!=null&&y===e.conversation.id,onClose:a,onActiveArchiveStart:f},e.key)),V=(0,Z.jsxs)(z.Section,{className:`vertical-scroll-fade-mask flex max-h-[60vh] flex-col gap-0 overflow-y-auto pb-1`,children:[B]});return(0,Z.jsxs)(`div`,{className:`flex max-h-[300px] w-[calc(var(--radix-popper-available-width)_-_var(--padding-panel))] flex-col gap-1`,children:[V]})}',
  'var messages={recentTasksMenu:1,search:{defaultMessage:`Search recent chats`}};function Sn(e){return e.kind===`remote`}function Cn(){}',
  'var An=(0,Dn.memo)(function(e){let t=(0,En.c)(24),{item:n,isActive:r,onClose:i,onActiveArchiveStart:a}=e;switch(n.kind){case`local`:{let e=null,c;return t[17]!==r||t[18]!==n.conversation.hostId||t[19]!==n.conversation.id||t[20]!==a||t[21]!==i||t[22]!==e?(c=(0,Z.jsx)(_t,{conversationId:n.conversation.id,hostId:n.conversation.hostId,isActive:r,metaContent:e,onClick:i,onActiveArchiveStart:a}),t[17]=r,t[18]=n.conversation.hostId,t[19]=n.conversation.id,t[20]=a,t[21]=i,t[22]=e,t[23]=c):c=t[23],c}}});',
  'const nativeMenu=(0,Q.jsx)(m,{contentClassName:`!pb-0 mt-[9px]`,triggerButton:se,open:g,onOpenChange:_,children:ce});',
  'function Bn(e){let t=(0,Gn.c)(64),{allowInitialRouteBack:r,className:i,centerContent:a,desktopDeepLinkConversationId:s,title:c,onBack:l,trailing:u}=e,d=r!==void 0&&r;return c}function Vn(){}',
].join('');
const split26581052044AppMainText = [
  'class Messenger{static getInstance(){return new Messenger}dispatchMessage(){}dispatchHostMessage(){}}',
  'var Zl=Messenger.getInstance();function useMessenger(){Zl.dispatchMessage(`native`,{}),Zl.dispatchHostMessage({type:`native`})}',
  'function ZC(e){return{activeWorkspaceRoot:`/project`,isActiveWorkspaceRootLoading:!1}}',
  'function iBe({userSavedModelString:e,userSavedReasoningEffort:t,listModelsData:n}){let r=n?.models?.find(n=>n.model===e),i=r?.supportedReasoningEfforts?.map(e=>e.reasoningEffort),a=t!=null&&i!=null&&i.includes(t)?t:r?.defaultReasoningEffort;return{model:r?.model,reasoningEffort:a}}',
  'function nativeState(m,t){let{isBackgroundSubagentsEnabled:s=!0}={},w=m?.model_reasoning_effort??null,n={model_reasoning_effort:t};return{isBackgroundSubagentsEnabled:s,w,n}}',
  'function nativeItems(n,o,d){switch(n.type){case`collabAgentToolCall`:{if(!o||n.tool===`wait`)break;let e={type:`multi-agent-action`,id:n.id};d.push(e);break}case`subAgentActivity`:if(!o)break;d.push({type:`subagent-activity`,id:n.id});break}}',
  'function fyn(e,t,n,r){let i=new Map;for(let[a,o]of e.entries())for(let e of r?.(t,o,a)??o.items){if(e.type===`subAgentActivity`){i.set(e.agentThreadId,{conversationId:e.agentThreadId,parentConversationId:t});continue}if(!(e.type!==`collabAgentToolCall`||e.tool!==`spawnAgent`))for(let r of e.receiverThreadIds)i.set(r,{conversationId:r,parentConversationId:t})}return Array.from(i.values())}',
  ...
  'function Nc(e,t){return t}var uJ=Nc($,(e,{get:t})=>{let n=t(lr,e);return dyn({cachedConversations:[],conversationTurns:n.turns,parentConversationId:e,sourceLinkedThreads:null}).filter(({conversationId:e})=>!t(blocked).includes(e))});',
  'function Fon(e,t){return e.flatMap((e,n)=>t?.some(t=>t.model===e.model&&t.supportedReasoningEfforts.some(({reasoningEffort:t})=>t===e.reasoningEffort))?[{...e,powerSettingIndex:n}]:[])}',
  'var Lon,Ron,zon;Lon=[{id:`gpt-5.6-terra:low`,model:`gpt-5.6-terra`,modelLabel:`5.6 Terra`,reasoningEffort:`low`},{id:`gpt-5.6-sol:xhigh`,model:`gpt-5.6-sol`,modelLabel:`5.6 Sol`,reasoningEffort:`xhigh`}],Ron={id:`gpt-5.6-sol:ultra`,model:`gpt-5.6-sol`,modelLabel:`5.6 Sol`,reasoningEffort:`ultra`},zon=[];',
  'function Aon(e,{includeUltraInSlider:t=!1,removeXHigh:n=!1}={}){let r=Fon((t?[...Lon,Ron]:Lon).filter(({reasoningEffort:e})=>!n||e!==`xhigh`),e);if(r.length>=3)return r;let i=Fon(zon.filter(({reasoningEffort:e})=>!n||e!==`xhigh`),e);return i.length>=3?i:[]}',
  'function TC(){return!0}var e4e=[];function l$(e,t){let n=e?.find(e=>e.model===t);return n==null?e4e.map(e=>({description:``,reasoningEffort:e})):n.supportedReasoningEfforts.filter(e=>TC(e.reasoningEffort))}',
  'function jc(e,t){return []}function POr(){}function Dzn(){}function q6(){return{jsx(){}}}function dCn(e){return e}',
  'function AOr(e){let t=(0,POr.c)(12),{activeConversationId:n,enabled:r,includeMentionItems:i}=e,a=jc(uJ,r?n:null),o,s;let parent=e=>e.parentConversationId===n,rw=a.filter(parent).filter(NOr);o=i?rw.map(MOr):[],s=rw.filter(jOr);let c=s,l=null;return{rows:a,visibleRows:c,mentionItems:o,firstApproval:l}}',
  'function jOr(e){return e.isCurrentParentTurn}function MOr(e){return{conversationId:e.conversationId,status:e.status}}function NOr(e){return e.canInteract&&e.displayName.trim().length>0}',
  'function Szn(e){let t=(0,Dzn.c)(40),{rows:n,agentCount:r,canStopAll:i,isStopAllDisabled:a,onOpenThread:o,onStopAll:s}=e;return{id:`composer.backgroundSubagents.summary`,rows:n,agentCount:r}}',
  'function cNr(){let ie=`x`,Ke=!0,Je={ui:{active:!0}},{rows:Ye,visibleRows:Xe}=AOr({activeConversationId:ie,enabled:Ke,includeMentionItems:Je.ui?.active===!0}),Ot=0,kt=!1,it=!1,pn=(Xe.length>0||kt)&&!it;dCn({subagentsPanel:pn});return pn?(0,q6.jsx)(Szn,{agentCount:Math.max(Xe.length,Ot),rows:Xe}):null}',
  'var app=`conversation.title untitledThreadLabel supportedReasoningEfforts defaultReasoningEffort networkConfig:{api:j} sdkExceptionUrl: safeGet makeRequest OAI-Language title:t(Bi,e) turns:t(Ote,e)`;',
  'export{Zl as kat,ZC as qZ,uJ as FT};',
].join('');
const split26581052044AppServerText = [
  'class Store{async loadThreadHydrationState(){}async listAllThreads({modelProviders:e,archived:t=!1,sourceKinds:n}){return gRt({sendRequest:this.params.requestClient.sendRequest.bind(this.params.requestClient),recentConversationsSortKey:this.params.requestClient.getCompatibleThreadSortKey(this.recentConversationSortKey)},{modelProviders:e,archived:t,sourceKinds:n})}async listArchivedThreads(){return[]}async listRecentThreads(){return{data:[],nextCursor:null}}}',
  'class Manager{async listAllThreads({modelProviders:e,archived:t=!1}){return this.threadStore.listAllThreads({modelProviders:e,archived:t})}async listArchivedThreads(){return this.threadStore.listArchivedThreads()}}',
  'function ssn(){return lsn(`recent-conversations`)}function lsn(e){return{data:[]}}',
  'function EF(e){return{createdAt:e.createdAt??0,updatedAt:e.updatedAt??0,recencyAt:e.recencyAt??null}}',
  'function zk(e){return{id:e.conversationId??e.id,cwd:e.cwd}}',
  'function kF(){return!0}function AF(e){return e}function wRt(e){return e.name??null}',
  'function MV(){return{getDefault:()=>({getHostId:()=>`local`}),getForHostId:()=>null}}',
  'function ZO(e){return e}function Zon(e){return()=>{for(let t of e)t()}}',
...
const header26581441407Text = [
  ...
  'function xn(e){let t=(0,En.c)(34),{cloudtasksQuery:n,localConversations:r,onClose:a,autoFocusSearch:o,showFilters:s}=e,c=o!==void 0&&o,l=s===void 0||s,u=y(),d=Ke(),{authMethod:f}=i(),T=e=>!0,C=null,v=null;let E=r.filter(T),D=hn(n.data,r,C),I=D,B=I.map(e=>(0,Z.jsx)(An,{item:e,isActive:e.kind===`local`&&e.conversation!=null&&v===e.conversation.id,onClose:a,onActiveArchiveStart:d},e.key)),V=(0,Z.jsxs)(z.Section,{className:`vertical-scroll-fade-mask flex max-h-[60vh] flex-col gap-0 overflow-y-auto pb-1`,children:[B]});return(0,Z.jsxs)(`div`,{className:`flex max-h-[300px] w-[calc(var(--radix-popper-available-width)_-_var(--padding-panel))] flex-col gap-1`,children:[V]})}',
  'var messages={recentTasksMenu:1,search:{defaultMessage:`Search recent chats`}};function Sn(e){return e.kind===`remote`}function Cn(){}',
  'var An=(0,Dn.memo)(function(e){let t=(0,En.c)(24),{item:n,isActive:r,onClose:i,onActiveArchiveStart:a}=e;switch(n.kind){case`local`:{let e=null,c;return t[17]!==r||t[18]!==n.conversation.hostId||t[19]!==n.conversation.id||t[20]!==a||t[21]!==i||t[22]!==e?(c=(0,Z.jsx)(ae,{conversationId:n.conversation.id,hostId:n.conversation.hostId,isActive:r,metaContent:e,onClick:i,onActiveArchiveStart:a}),t[17]=r,t[18]=n.conversation.hostId,t[19]=n.conversation.id,t[20]=a,t[21]=i,t[22]=e,t[23]=c):c=t[23],c}}});',
  'const nativeMenu=(0,Q.jsx)(m,{contentClassName:`!pb-0 mt-[9px]`,triggerButton:ie,open:g,onOpenChange:_,children:ce});',
  'function zn(e){let t=(0,Wn.c)(64),{allowInitialRouteBack:r,className:i,centerContent:a,desktopDeepLinkConversationId:o,title:s,onBack:c,trailing:l}=e,u=r!==void 0&&r;return s}function Vn(){}',
].join('');
const split26581441407AppMainText = [
  'class Messenger{static getInstance(){return new Messenger}dispatchMessage(){}dispatchHostMessage(){}}',
  'var Zl=Messenger.getInstance();function useMessenger(){Zl.dispatchMessage(`native`,{}),Zl.dispatchHostMessage({type:`native`})}',
  'function ZC(e){return{activeWorkspaceRoot:`/project`,isActiveWorkspaceRootLoading:!1}}',
  'function iBe({userSavedModelString:e,userSavedReasoningEffort:t,listModelsData:n}){let r=n?.models?.find(n=>n.model===e),i=r?.supportedReasoningEfforts?.map(e=>e.reasoningEffort),a=t!=null&&i!=null&&i.includes(t)?t:r?.defaultReasoningEffort;return{model:r?.model,reasoningEffort:a}}',
  'function nativeState(m,t){let{isBackgroundSubagentsEnabled:s=!0}={},w=m?.model_reasoning_effort??null,n={model_reasoning_effort:t};return{isBackgroundSubagentsEnabled:s,w,n}}',
  'function nativeItems(n,o,d){switch(n.type){case`collabAgentToolCall`:{if(!o||n.tool===`wait`)break;let e={type:`multi-agent-action`,id:n.id};d.push(e);break}case`subAgentActivity`:if(!o)break;d.push({type:`subagent-activity`,id:n.id});break}}',
  'function js(e){return String(e)}function SNn(e,t,n,r){let i=new Map;for(let[a,o]of e.entries())for(let e of r?.(t,o,a)??o.items){if(e.type===`subAgentActivity`){let r=js(e.agentThreadId),a=n?.get(r);i.set(r,{conversationId:r,displayName:null,parentConversationId:t});continue}if(!(e.type!==`collabAgentToolCall`||e.tool!==`spawnAgent`))for(let r of e.receiverThreadIds){let e=js(r),a=n?.get(e);i.has(e)||i.set(e,{conversationId:e,displayName:null,parentConversationId:t})}}return Array.from(i.values())}',
  'function xNn({cachedConversations:e,conversationTurns:t,getIndexedSubagentItems:n,parentConversationId:a}){let d=SNn(t,a,null,n).map(e=>e);return d}',
  'function Ri(e,t){return t}var lX=Ri($,(e,{get:t})=>{if(e==null)return[];let n=typeof e==`string`?e:e.conversationId,r=typeof e==`string`?void 0:e.parentTurnKey,a=t(store,n),b=xNn({cachedConversations:[],conversationTurns:a.turns,getIndexedSubagentItems:null,parentConversationId:n});return b},{isEqual:eq});',
  'function Jdn(e,t){return e.flatMap((e,n)=>t?.some(t=>t.model===e.model&&t.supportedReasoningEfforts.some(({reasoningEffort:t})=>t===e.reasoningEffort))?[{...e,powerSettingIndex:n}]:[])}',
  'var Xdn,Zdn,zdn;Xdn=[{id:`gpt-5.6-terra:low`,model:`gpt-5.6-terra`,modelLabel:`5.6 Terra`,reasoningEffort:`low`},{id:`gpt-5.6-sol:xhigh`,model:`gpt-5.6-sol`,modelLabel:`5.6 Sol`,reasoningEffort:`xhigh`}],Zdn={id:`gpt-5.6-sol:ultra`,model:`gpt-5.6-sol`,modelLabel:`5.6 Sol`,reasoningEffort:`ultra`},zdn=[];',
  'function Udn(e,{includeUltraInSlider:t=!1,removeXHigh:n=!1}={}){let r=Jdn((t?[...Xdn,Zdn]:Xdn).filter(({reasoningEffort:e})=>!n||e!==`xhigh`),e);if(r.length>=3)return r;let i=Jdn(zdn.filter(({reasoningEffort:e})=>!n||e!==`xhigh`),e);return i.length>=3?i:[]}',
  'function NC(){return!0}var q3e=[];function S$(e,t){let n=e?.find(e=>e.model===t);return n==null?q3e.map(e=>({description:``,reasoningEffort:e})):n.supportedReasoningEfforts.filter(e=>NC(e.reasoningEffort))}',
  'function sl(e,t){return []}function UBr(){}function I6(){return{jsx(){}}}function QFn(e){return e}',
  'function zBr(e){let t=(0,UBr.c)(12),{activeConversationId:n,enabled:r,includeMentionItems:i}=e,a=sl(lX,r?n:null),o,s;let parent=e=>e.parentConversationId===n,rw=a.filter(parent).filter(HBr);o=i?rw.map(VBr):[],s=rw.filter(BBr);let c=s;return{rows:a,visibleRows:c,mentionItems:o}}',
  'function BBr(e){return e.isCurrentParentTurn}function VBr(e){return e}function HBr(e){return e.canInteract&&e.displayName.trim().length>0}',
  'function RQn(e){let{rows:n,agentCount:r}=e;return{id:`composer.backgroundSubagents.summary`,rows:n,agentCount:r}}',
  'function vWr(){let {rows:nt,visibleRows:rt}=zBr({activeConversationId:ae,enabled:$e,includeMentionItems:tt.ui?.active===!0}),Ft=nt.length,It=Ft>0,dt=!1,hn=!1,ht=!1,pt=!1,yn=(rt.length>0||It)&&!dt&&!hn&&!ht&&!pt;let layout=QFn({subagentsPanel:yn});if(a){if(b){if(c){return yn?(0,I6.jsx)(RQn,{agentCount:Math.max(rt.length,Ft),rows:rt}):null}}}return null}',
  ...
  'export{Zl as Vst,ZC as U$,lX as IC};',
].join('');
const split26581441407AppServerText = [
  'class Store{async loadThreadHydrationState(){}async listAllThreads({modelProviders:e,archived:t=!1,sourceKinds:n}){return mnt({sendRequest:this.params.requestClient.sendRequest.bind(this.params.requestClient),recentConversationsSortKey:this.params.requestClient.getCompatibleThreadSortKey(this.recentConversationSortKey)},{modelProviders:e,archived:t,sourceKinds:n})}async listArchivedThreads(){return[]}async listRecentThreads(){return{data:[],nextCursor:null}}}',
  'class Manager{async listAllThreads({modelProviders:e,archived:t=!1}){return this.threadStore.listAllThreads({modelProviders:e,archived:t})}async listArchivedThreads(){return this.threadStore.listArchivedThreads()}}',
  'function dgn(){return pgn(`recent-conversations`)}function pgn(e){return{data:[]}}',
  'function PD(e){return{createdAt:e.createdAt??0,updatedAt:e.updatedAt??0,recencyAt:e.recencyAt??null}}',
  'function nO(e){return{id:e.conversationId??e.id,cwd:e.cwd}}',
  'function bot(){return!0}function AF(e){return e}function wRt(e){return e.name??null}',
  'function WV(){return{getDefault:()=>({getHostId:()=>`local`}),getForHostId:()=>null}}',
  'function vM(e){return e}function tgn(e){return()=>{for(let t of e)t()}}',
  'function ngn({appServerRegistry:e,onStoreChange:t,subscribeToManager:n}){return()=>{}}',
  'var KV={useEffect(){}};var recentConversationsSortKey=1,threadList=`thread/list`;',
].join('');
const FIXTURES_265810 = {
  41047: { header: header26581041047Text, main: split26581041047AppMainText, server: split26581041047AppServerText },
  52044: { header: header26581052044Text, main: split26581052044AppMainText, server: split26581052044AppServerText },
};

function configure265810Features(target, build) {
  const fixture = FIXTURES_265810[build || '52044'];
  const assets = path.dirname(target.headerPath);
  target.version = '26.5810.' + (build || '52044');
  target.appStatsigPath = target.appMainPath;
  fs.writeFileSync(target.extensionJsPath, extensionText
    .replace('e.onDidReceiveMessage(n=>{let o=a2(n);o!=null&&this.#a(o.message)})', 'e.onDidReceiveMessage(n=>{let o=B8(n);o==null||o.sessionId!==this.#r||this.#a(o.message)})')
    .replace('e.onDidReceiveMessage(a=>{if(a.type==="ready"){o?.()}this.handleMessage(e,a)})', 'e.onDidReceiveMessage(c=>{if(c.type==="ready"){o?.()}this.handleMessage(e,c)})') + webviewTimeout265810Text);
  fs.writeFileSync(target.headerPath, fixture.header);
  fs.writeFileSync(target.appMainPath, fixture.main);
  fs.writeFileSync(target.appServerManagerSignalsPath, fixture.server);
  fs.writeFileSync(path.join(assets, 'app-initial-main265810.js'), fixture.main);
  fs.writeFileSync(path.join(assets, 'app-initial-server265810.js'), fixture.server);
}

function configure265814Features(target) {
  const assets = path.dirname(target.headerPath);
  target.version = '26.5814.41407';
  target.appStatsigPath = target.appMainPath;
  fs.writeFileSync(target.extensionJsPath, extensionText
    .replace('e.onDidReceiveMessage(n=>{let o=a2(n);o!=null&&this.#a(o.message)})', 'e.onDidReceiveMessage(n=>{let o=Q9(n);o==null||o.sessionId!==this.#r||this.#a(o.message)})')
    .replace('e.onDidReceiveMessage(a=>{if(a.type==="ready"){o?.()}this.handleMessage(e,a)})', 'e.onDidReceiveMessage(c=>{if(c.type==="ready"){o?.()}this.handleMessage(e,c)})') + webviewTimeout265814Text);
  fs.writeFileSync(target.headerPath, header26581441407Text);
  fs.writeFileSync(target.appMainPath, split26581441407AppMainText);
  fs.writeFileSync(target.appServerManagerSignalsPath, split26581441407AppServerText);
  fs.writeFileSync(path.join(assets, 'app-initial-main265814.js'), split26581441407AppMainText);
...
const CODEX_265810_TEST_VARIANTS = [
  {
    build: '41047',
    historyHook: 'Ron',
    sliderFn: 'kon',
    menuFn: 'u$',
    ultraObjectClose: '}],Lon=',
    producerDrifts: [
      ['e.type===`subAgentActivity`', 'e.type===`brokenActivity`'],
      ['e.tool!==`spawnAgent`', 'e.tool!==`brokenSpawn`'],
      ['i.set(e.agentThreadId,{conversationId:e.agentThreadId,parentConversationId:t})', 'i.set(e.agentThreadId,{conversationId:e.agentThreadId,parentConversationId:other})'],
      ['i.set(r,{conversationId:r,parentConversationId:t})', 'i.set(r,{conversationId:r,parentConversationId:other})'],
      ['let i=dyn(t,a,null,null).map(e=>e)', 'let i=[]'],
      ['return uyn({cachedConversations:[],conversationTurns:n.turns,parentConversationId:e,sourceLinkedThreads:null}).filter', 'return [].filter'],
      ['lJ as FT', 'lJ as broken'],
    ],
    composerDrifts: [
      ['a=wc(lJ,r?n:null)', 'a=wc(broken,r?n:null)'],
  ...
  },
  {
    ...
    sliderFn: 'Aon',
    menuFn: 'l$',
    ultraObjectClose: '}],Ron=',
    producerDrifts: [
      ['e.type===`subAgentActivity`', 'e.type===`brokenActivity`'],
      ['e.tool!==`spawnAgent`', 'e.tool!==`brokenSpawn`'],
      ['i.set(e.agentThreadId,{conversationId:e.agentThreadId,parentConversationId:t})', 'i.set(e.agentThreadId,{conversationId:e.agentThreadId,parentConversationId:other})'],
      ['i.set(r,{conversationId:r,parentConversationId:t})', 'i.set(r,{conversationId:r,parentConversationId:other})'],
      ['let i=fyn(t,a,null,null).map(e=>e)', 'let i=[]'],
      ['return dyn({cachedConversations:[],conversationTurns:n.turns,parentConversationId:e,sourceLinkedThreads:null}).filter', 'return [].filter'],
      ['uJ as FT', 'uJ as broken'],
    ],
    composerDrifts: [
      ['a=jc(uJ,r?n:null)', 'a=jc(broken,r?n:null)'],
      ['parentConversationId===n', 'parentConversationId===other'],
...
function codex265810BuildTests() {
  return CODEX_265810_TEST_VARIANTS.flatMap((v) => [
    {
      name: `adapts Codex 26.5810.${v.build} host, header and idempotent plan`,
      run() {
        const target = createTarget();
        configure265810Features(target, v.build);
        ...
        assert.ok(bundles[target.extensionJsPath].includes('if(codexLocalGroupsHandleWebviewMessage(n))return;let o=B8(n)'));
        assert.ok(bundles[target.headerPath].includes('codexLocalGroupsHeaderSafe265810PatchVersion=1'));
        assert.ok(bundles[target.headerPath].includes('codexLocalGroupsOpenedTitle265810PatchVersion=1'));
        assert.ok(bundles[target.headerPath].includes('titleOverride:codexLocalGroupsLocalTitle(n)'));
        assert.ok(bundles[target.appMainPath].includes('codexLocalGroupsCodexUi265810PatchVersion=1'));
        assert.ok(bundles[target.appServerManagerSignalsPath].includes('function ' + v.historyHook + '(e,t,n)'));
    ...
    {
      name: `executes Codex 26.5810.${v.build} Sol Max Ultra and isolated project history`,
      run() {
        const target = createTarget();
        configure265810Features(target, v.build);
        ...
        assert.ok(appMain.includes('gpt-5.6-sol:max'));
        assert.strictEqual(appMain.split('{id:`gpt-5.6-sol:ultra`').length - 1, 1);
        const main = appMain.replace(/export\{[^}]+\};?/, '');
        const mainScript = `var $;${main};let efforts=value=>value.map(reasoningEffort=>({reasoningEffort})),models=[{model:'gpt-5.6-sol',supportedReasoningEfforts:efforts(['xhigh']),defaultReasoningEffort:'xhigh'},{model:'gpt-5.6-terra',supportedReasoningEfforts:efforts(['low']),defaultReasoningEffort:'low'}];console.log(JSON.stringify({sol:iBe({userSavedModelString:'gpt-5.6-sol',userSavedReasoningEffort:'max',listModelsData:{models}}).reasoningEffort,terra:iBe({userSavedModelString:'gpt-5.6-terra',userSavedReasoningEffort:'max',listModelsData:{models}}).reasoningEffort,power:${v.sliderFn}(models).map(e=>e.reasoningEffort),menu:${v.menuFn}(models,'gpt-5.6-sol').map(e=>e.reasoningEffort)}))`;
        ...
        assert.deepStrictEqual(JSON.parse(mainResult.stdout), { sol: 'max', terra: 'low', power: ['low', 'xhigh', 'max', 'ultra'], menu: ['xhigh', 'max', 'ultra'] });
        const serverScript = `${server.replace(/export\{[^}]+\};?/, '')};(async()=>{let pages={first:{data:[{id:'root',cwd:'/project',updatedAt:3},{id:'other',cwd:'/project2',updatedAt:2}],nextCursor:'next'},next:{data:[{id:'child',cwd:'/project/sub',updatedAt:1}],nextCursor:null}},store={threadsById:new Map,listRecentThreads:({cursor})=>Promise.resolve(cursor==null?pages.first:pages.next),getThreadSummaryFromThread:e=>({conversationId:e.id,cwd:e.cwd,updatedAt:e.updatedAt}),shouldSurfaceThreadSummary:()=>true},project=await codexLocalGroupsLoadProjectConversations265810(store,'/project');console.log(JSON.stringify(project.map(e=>e.id)))})()`;
    ...
    {
      name: `fails closed when Codex 26.5810.${v.build} loses the unique jP watchdog`,
      run() {
        const target = createTarget();
        configure265810Features(target, v.build);
        ...
        const plan = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true }).plan(target, { version: 1, conversations: {} });
        assert.ok(plan.errors.includes('extension webview startup timeout 26.5810: 找不到唯一 jP 看门狗'));
      },
    },
    {
      name: `fails closed when Codex 26.5810.${v.build} Power Max anchor drifts`,
      run() {
        const target = createTarget();
        configure265810Features(target, v.build);
        fs.writeFileSync(target.appMainPath, FIXTURES_265810[v.build].main.replace('reasoningEffort:`xhigh`' + v.ultraObjectClose, 'reasoningEffort:`xhigh`}],Broken='));
        fs.writeFileSync(path.join(path.dirname(target.headerPath), 'app-initial-main265810.js'), fs.readFileSync(target.appMainPath, 'utf8'));
        const plan = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true }).plan(target, { version: 1, conversations: {} });
        assert.ok(plan.errors.some((error) => error.includes('Codex power 26.5810 Sol Max option')));
      },
    },
    {
      name: `executes Codex 26.5810.${v.build} metadata four entries including promptNewGroup`,
      run() {
        const target = createTarget();
        configure265810Features(target, v.build);
        const plan = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true }).plan(target, { version: 1, conversations: {} });
    ...
    {
      name: `executes Codex 26.5810.${v.build} same-group 25-row 15-to-5 collapse and hidden counts`,
      run() {
        const target = createTarget();
        configure265810Features(target, v.build);
        const plan = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true }).plan(target, { version: 1, conversations: {} });
        ...
        const probe = runHeaderRows(header, null, { items, metadata, currentRoot: '/project', includeStorage: true, toggleMore: true, toggleMoreTwice: true });
        const at5 = JSON.stringify(probe.rows);
        const at15 = JSON.stringify(probe.expandedRows);
        const atBack = JSON.stringify(probe.collapsedRows);
        assert.strictEqual((at5.match(/"type":"CodexRow"/g) || []).length, 5);
        assert.ok(at5.includes('还有 20 条，展开更多'));
        assert.ok(!at5.includes('收起到最近 5 条'));
        assert.strictEqual((at15.match(/"type":"CodexRow"/g) || []).length, 15);
        assert.ok(at15.includes('还有 10 条，展开更多'));
        assert.ok(at15.includes('收起到最近 5 条'));
        assert.ok(!at15.includes('收起到最近 15 条'));
        const at25 = JSON.stringify(probe.fullRows);
        assert.strictEqual((at25.match(/"type":"CodexRow"/g) || []).length, 25);
        assert.ok(at25.includes('收起到最近 15 条'));
        assert.ok(at25.includes('收起到最近 5 条'));
        assert.ok(!at25.includes('还有 '));
        assert.strictEqual((atBack.match(/"type":"CodexRow"/g) || []).length, 5);
        assert.ok(atBack.includes('还有 20 条，展开更多'));
      },
    },
    {
      name: `executes Codex 26.5810.${v.build} same-id dropdown and Bn titles with native fallback`,
      run() {
        const target = createTarget();
        configure265810Features(target, v.build);
        const plan = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true }).plan(target, { version: 1, conversations: {} });
        const header = plan.changes.find((change) => change.path === target.headerPath).nextText;
        const items = [
          { kind: 'local', key: 'abc', conversation: { id: 'abc', hostId: 'local', cwd: '/project', title: '切换到指定 Git 分支' } },
          { kind: 'local', key: 'missing', conversation: { id: 'missing', hostId: 'local', cwd: '/project', title: '切换到指定 Git 分支' } },
          { kind: 'local', key: 'empty', conversation: { id: 'empty', hostId: 'local', cwd: '/project', title: '切换到指定 Git 分支' } },
        ];
        const metadata = { version: 1, conversations: { abc: { title: '积分订单导出修改', group: '需求A', projectRoot: '/project' }, empty: { title: '   ', group: '需求A', projectRoot: '/project' } } };
        const probe = runHeaderRows(header, 'abc', { items, metadata, currentRoot: '/project', includeStorage: true });
        const drop = JSON.stringify(probe.rows);
        assert.ok(drop.includes('积分订单导出修改'));
        assert.strictEqual((drop.match(/切换到指定 Git 分支/g) || []).length, 2);
        const pageHeader = header.slice(header.indexOf('function Bn(e){'), header.indexOf('function Vn()', header.indexOf('function Bn(e){')));
        const script = `const listeners={},window={addEventListener:(name,fn)=>listeners[name]=fn,removeEventListener:(name,fn)=>listeners[name]===fn&&delete listeners[name]},codexLocalGroupsLocalTitle=({conversation})=>conversation.id==='abc'?'积分订单导出修改':null,In={useState:()=>[0,fn=>globalThis.refresh=fn(0)],useEffect:fn=>globalThis.cleanup=fn()},Gn={c:()=>[]},$={jsx:(type,props)=>props.children,jsxs:(type,props)=>props.children};globalThis.refresh=null;${pageHeader};const local=Bn({desktopDeepLinkConversationId:'abc',title:'切换到指定 Git 分支'}),missing=Bn({desktopDeepLinkConversationId:'missing',title:'切换到指定 Git 分支'}),empty=Bn({desktopDeepLinkConversationId:'empty',title:'切换到指定 Git 分支'});listeners['codex-local-groups-refresh']();cleanup();console.log(JSON.stringify([local,missing,empty,refresh,listeners['codex-local-groups-refresh']??null]))`;
    ...
    {
      name: `fails closed when the 26.5810.${v.build} dropdown title consumer is incomplete`,
      run() {
        const target = createTarget();
        configure265810Features(target, v.build);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        ...
        const broken = header.replace('titleOverride:codexLocalGroupsLocalTitle(n)?(0,Z.jsx)(Z.Fragment,{children:n.conversation.title}):void 0,', '');
        assert.notStrictEqual(broken, header);
        fs.writeFileSync(target.headerPath, broken);
        const plan = engine.plan(target, { version: 1, conversations: {} });
        assert.ok(plan.errors.includes('header 26.5810: 补丁标记不完整'));
      },
    },
    {
      name: `fails closed when the 26.5810.${v.build} producer chain is broken`,
      run() {
        for (const [before, after] of v.producerDrifts) {
          const target = createTarget();
          configure265810Features(target, v.build);
          const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
          for (const change of engine.plan(target, { version: 1, conversations: {} }).changes) fs.writeFileSync(change.path, change.nextText);
          const current = fs.readFileSync(target.appMainPath, 'utf8');
          fs.writeFileSync(target.appMainPath, current.replace(before, after));
          assert.ok(engine.plan(target, { version: 1, conversations: {} }).errors.includes('Codex UI 26.5810: 补丁标记不完整'), before);
        }
      },
    },
    {
      name: `fails closed when the 26.5810.${v.build} composer chain is broken`,
      run() {
  ...
  tests: [
    ...
    },
    {
      ...
      run() {
        ...
        assert.ok(main.includes('collabAgentToolCall'));
        assert.ok(main.includes('multi-agent-action'));
        assert.ok(extension.includes('codexLocalGroupsHandleWebviewMessage(c,e)'));
        assert.ok(extension.includes('this.onTimeout()},12e4))'));
        assert.deepStrictEqual(run265730HeaderActions(header), {
          dispatched: [
    ...
    },
    {
      ...
      run() {
        ...
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        for (const change of engine.plan(target, { version: 1, conversations: {} }).changes) fs.writeFileSync(change.path, change.nextText);
        const text = fs.readFileSync(target.appMainPath, 'utf8')
          .replace('type:`multi-agent-action`', 'type:`missing-v1-action`')
          .replace('type:`subagent-activity`', 'legacyType:`multi-agent-action`,type:`subagent-activity`');
    ...
    {
      name: 'fails closed when the V1 action object is not pushed',
      run() {
        const target = createTarget();
        configure265803Features(target);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        for (const change of engine.plan(target, { version: 1, conversations: {} }).changes) fs.writeFileSync(change.path, change.nextText);
        const text = fs.readFileSync(target.appMainPath, 'utf8').replace('d.push(e);break}case`subAgentActivity`', 'void e;break}case`subAgentActivity`');
        fs.writeFileSync(target.appMainPath, text);
        const plan = engine.plan(target, { version: 1, conversations: {} });
        assert.ok(plan.errors.includes('Codex UI 26.5803: 补丁标记不完整'));
      },
    ...
    {
      name: 'fails closed when the V1 action id comes from another input',
      run() {
        const target = createTarget();
        configure265803Features(target);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        for (const change of engine.plan(target, { version: 1, conversations: {} }).changes) fs.writeFileSync(change.path, change.nextText);
        const text = fs.readFileSync(target.appMainPath, 'utf8').replace('id:n.id};d.push(e)', 'id:other.id};d.push(e)');
        fs.writeFileSync(target.appMainPath, text);
        const plan = engine.plan(target, { version: 1, conversations: {} });
        assert.ok(plan.errors.includes('Codex UI 26.5803: 补丁标记不完整'));
      },
    ...
    {
      name: 'fails closed when the V1 gate reads another input',
      run() {
        const target = createTarget();
        configure265803Features(target);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        for (const change of engine.plan(target, { version: 1, conversations: {} }).changes) fs.writeFileSync(change.path, change.nextText);
        const text = fs.readFileSync(target.appMainPath, 'utf8').replace('n.tool===`wait`', 'other.tool===`wait`');
        fs.writeFileSync(target.appMainPath, text);
        const plan = engine.plan(target, { version: 1, conversations: {} });
        assert.ok(plan.errors.includes('Codex UI 26.5803: 补丁标记不完整'));
      },
    ...
    {
      name: 'fails closed when V2 output only appears in the default branch',
      run() {
        const target = createTarget();
        configure265803Features(target);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        for (const change of engine.plan(target, { version: 1, conversations: {} }).changes) fs.writeFileSync(change.path, change.nextText);
        const text = fs.readFileSync(target.appMainPath, 'utf8')
          .replace('type:`subagent-activity`', 'type:`missing-v2-action`')
          .replace('break}}var app', 'break;default:d.push({type:`subagent-activity`});break}}var app');
        fs.writeFileSync(target.appMainPath, text);
        const plan = engine.plan(target, { version: 1, conversations: {} });
        assert.ok(plan.errors.includes('Codex UI 26.5803: 补丁标记不完整'));
      },
    ...
    {
      name: 'fails closed when V2 output only appears after the switch closes',
      run() {
        const target = createTarget();
        configure265803Features(target);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        ...
        const text = fs.readFileSync(target.appMainPath, 'utf8')
          .replace('type:`subagent-activity`', 'type:`missing-v2-action`')
          .replace('break}}var app', 'break}}var unrelated={type:`subagent-activity`};var app');
        fs.writeFileSync(target.appMainPath, text);
        const plan = engine.plan(target, { version: 1, conversations: {} });
        assert.ok(plan.errors.includes('Codex UI 26.5803: 补丁标记不完整'));
      },
    ...
    {
      name: 'fails closed when the V2 activity id comes from another input',
      run() {
        const target = createTarget();
        configure265803Features(target);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        for (const change of engine.plan(target, { version: 1, conversations: {} }).changes) fs.writeFileSync(change.path, change.nextText);
        const text = fs.readFileSync(target.appMainPath, 'utf8').replace('type:`subagent-activity`,id:n.id', 'type:`subagent-activity`,id:other.id');
        fs.writeFileSync(target.appMainPath, text);
        const plan = engine.plan(target, { version: 1, conversations: {} });
        assert.ok(plan.errors.includes('Codex UI 26.5803: 补丁标记不完整'));
      },
    ...
    {
      name: 'adapts Codex 26.5803.61601 model selector symbol drift',
      run() {
        const target = createTarget();
        configure265803Features(target, '26.5803.61601', split265803LatestAppMainText);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        const plan = engine.plan(target, { version: 1, conversations: {} });
        assert.deepStrictEqual(plan.errors, []);
        assert.deepStrictEqual(new Set(plan.changes.map((change) => change.path)), new Set([
          target.extensionJsPath,
          target.headerPath,
          target.appMainPath,
        ...
        assert.ok(appMain.includes('codexLocalGroupsCodexUi265803PatchVersion=1;function iBe({userSavedModelString:'));
        assert.ok(appMain.includes('e?RS(n?.models,e):n?.defaultModel??RS(n?.models,`gpt-5.5`)'));
        assert.deepStrictEqual(engine.plan(target, { version: 1, conversations: {} }), { changes: [], errors: [], unsafeBundles: [] });
      },
    },
    {
      name: 'fails closed without writing when Codex 26.5803 loses the model selector signature',
      run() {
        const target = createTarget();
        configure265803Features(target, '26.5803.61601', split265803LatestAppMainText.replace('userSavedModelString', 'missingSavedModelString'));
        const before = fs.readFileSync(target.appMainPath, 'utf8');
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        const result = engine.apply(target, { version: 1, conversations: {} });
        assert.ok(result.errors.includes('Codex UI 26.5803 marker: 期望 1 处匹配，实际 0 处'));
        assert.strictEqual(result.idempotent, false);
        assert.strictEqual(fs.readFileSync(target.appMainPath, 'utf8'), before);
      },
    },
    {
      name: 'fails closed when Codex 26.5803 has multiple model selector signatures',
      run() {
        const target = createTarget();
        configure265803Features(target, '26.5803.61601', `${split265803LatestAppMainText}function duplicate({userSavedModelString:e}){}`);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        const plan = engine.plan(target, { version: 1, conversations: {} });
        assert.ok(plan.errors.includes('Codex UI 26.5803 marker: 期望 1 处匹配，实际 2 处'));
      },
    },
    {
    ...
    ...codex265810BuildTests(),
    {
      name: 'fails closed on an unknown Codex 26.5810 build',
      run() {
        const target = createTarget();
        configure265810Features(target, '52044');
        target.version = '26.5810.53000';
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        const plan = engine.plan(target, { version: 1, conversations: {} });
    ...
    {
      name: 'recognizes the verified Codex 26.5814 build',
      run() {
        const target = createTarget();
        configure265814Features(target);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        const applied = engine.apply(target, { version: 1, conversations: {} });
    ...
    {
      name: 'executes Codex 26.5814 metadata four entries',
      run() {
        const target = createTarget();
        configure265814Features(target);
        const plan = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true }).plan(target, { version: 1, conversations: {} });
    ...
    {
      name: 'executes Codex 26.5814 same-group 25-row 15-to-5 collapse and hidden counts',
      run() {
        const target = createTarget();
        configure265814Features(target);
        const plan = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true }).plan(target, { version: 1, conversations: {} });
        ...
        assert.deepStrictEqual(states.map((value) => (value.match(/"type":"CodexRow"/g) || []).length), [5, 15, 25, 5]);
        assert.ok(states[0].includes('还有 20 条，展开更多'));
        assert.ok(states[1].includes('还有 10 条，展开更多') && states[1].includes('收起到最近 5 条'));
        assert.ok(states[2].includes('收起到最近 15 条') && states[2].includes('收起到最近 5 条') && !states[2].includes('还有 '));
        assert.ok(states[3].includes('还有 20 条，展开更多'));
      },
    },
    {
      name: 'executes Codex 26.5814 same-id dropdown and zn titles with native fallback',
      run() {
        const target = createTarget();
        configure265814Features(target);
        const plan = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true }).plan(target, { version: 1, conversations: {} });
    ...
    {
      name: 'executes Codex 26.5814 Sol and isolated project history contracts',
      run() {
        const target = createTarget();
        configure265814Features(target);
        const plan = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true }).plan(target, { version: 1, conversations: {} });
        ...
        const mainScript = `var $,eq;${main};let efforts=value=>value.map(reasoningEffort=>({reasoningEffort})),models=[{model:'gpt-5.6-sol',supportedReasoningEfforts:efforts(['xhigh']),defaultReasoningEffort:'xhigh'},{model:'gpt-5.6-terra',supportedReasoningEfforts:efforts(['low']),defaultReasoningEffort:'low'}],reasoning=effort=>iBe({userSavedModelString:'gpt-5.6-sol',userSavedReasoningEffort:effort,listModelsData:{models}}).reasoningEffort;console.log(JSON.stringify({max:reasoning('max'),ultra:reasoning('ultra'),readMax:nativeState({model_reasoning_effort:'max'},'max').w,readUltra:nativeState({model_reasoning_effort:'ultra'},'ultra').w,writeMax:nativeState({},'max').n.model_reasoning_effort,writeUltra:nativeState({},'ultra').n.model_reasoning_effort,power:Udn(models).map(e=>e.reasoningEffort),menu:S$(models,'gpt-5.6-sol').map(e=>e.reasoningEffort)}))`;
        const mainResult = childProcess.spawnSync(resolveNodePath(), ['-e', mainScript], { encoding: 'utf8' });
        assert.strictEqual(mainResult.status, 0, mainResult.stderr);
    ...
    {
      name: 'patches and verifies the 26.5814 Q9 webview message parser',
      run() {
        const target = createTarget();
        configure265814Features(target);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        const plan = engine.plan(target, { version: 1, conversations: {} });
        const extension = plan.changes.find((change) => change.path === target.extensionJsPath).nextText;
        assert.ok(extension.includes('if(codexLocalGroupsHandleWebviewMessage(n))return;let o=Q9(n)'));
        fs.writeFileSync(target.extensionJsPath, extension.replace('let o=Q9(n)', 'let o=Qx(n)'));
        const broken = engine.plan(target, { version: 1, conversations: {} });
        assert.ok(broken.errors.includes('extension capn metadata message handler 26.5814: 补丁后置条件不完整'), JSON.stringify(broken.errors));
      },
    },
    {
      name: 'fails closed when the 26.5814 subagent selector parent binding drifts',
      run() {
        const target = createTarget();
        configure265814Features(target);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        const applied = engine.apply(target, { version: 1, conversations: {} });
        assert.deepStrictEqual(applied.errors, []);
        const patchedMain = fs.readFileSync(target.appMainPath, 'utf8');
        const drifts = [
          ['e:e.conversationId', 'e:broken'],
          ['parentConversationId:n})', 'parentConversationId:broken})'],
        ];
        for (const [before, after] of drifts) {
          fs.writeFileSync(target.appMainPath, patchedMain.replace(before, after));
          const broken = engine.plan(target, { version: 1, conversations: {} });
          assert.ok(broken.errors.includes('Codex UI 26.5814: 补丁标记不完整'), before);
        }
      },
    },
    {
      ...
      run() {
        ...
        const header = fs.readFileSync(target.headerPath, 'utf8');
        for (const marker of ['promptConversationTitle', 'promptConversationGroup', 'promptNewGroup', 'setPendingGroup']) {
          fs.writeFileSync(target.headerPath, header.replace('action:`' + marker + '`', 'action:`broken`'));
          assert.ok(engine.plan(target, { version: 1, conversations: {} }).errors.includes('header 26.5814: 补丁标记不完整'), marker);
        }
        for (const marker of ['dispatchHostMessage({type:`new-chat`})', 't.action===`metadataSaved`']) {
          fs.writeFileSync(target.headerPath, header.replace(marker, 'brokenMetadataContract'));
          assert.ok(engine.plan(target, { version: 1, conversations: {} }).errors.includes('header 26.5814: 补丁标记不完整'), marker);
        }
      },
    },
    {
      name: 'rejects 26.5814 metadata actions moved into later decoy functions',
      run() {
        const target = createTarget();
        configure265814Features(target);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        ...
        for (const action of ['promptConversationTitle', 'promptConversationGroup', 'promptNewGroup', 'setPendingGroup']) {
          const broken = header.replace('action:`' + action + '`', 'action:`broken`') + `function laterMetadataDecoy(){return{action:\`${action}\`}}`;
          fs.writeFileSync(target.headerPath, broken);
          assert.ok(engine.plan(target, { version: 1, conversations: {} }).errors.includes('header 26.5814: 补丁标记不完整'), action);
        }
      },
    },
    {
      name: 'rejects 26.5814 metadata actions and callbacks inside nested decoy functions',
      run() {
        const target = createTarget();
        configure265814Features(target);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        assert.deepStrictEqual(engine.apply(target, { version: 1, conversations: {} }).errors, []);
        const patchedHeader = fs.readFileSync(target.headerPath, 'utf8');
        const header = patchedHeader.replace('action:`promptConversationTitle`', 'action:`broken`');
        const headerDecoy = 'function nestedMetadataDecoy(){try{codexLocalGroupsMessenger.dispatchMessage(`codex-local-groups`,{action:`promptConversationTitle`})}catch{}}';
        fs.writeFileSync(target.headerPath, header.replace('function codexLocalGroupsPromptTitle(e,t,n){', 'function codexLocalGroupsPromptTitle(e,t,n){' + headerDecoy));
        assert.ok(engine.plan(target, { version: 1, conversations: {} }).errors.includes('header 26.5814: 补丁标记不完整'));
        const sameTry = header.replace('function codexLocalGroupsPromptTitle(e,t,n){try{', 'function codexLocalGroupsPromptTitle(e,t,n){try{({action:`promptConversationTitle`,conversationId:String(e),title:t??``,projectRoot:n??``});');
        fs.writeFileSync(target.headerPath, sameTry);
        assert.ok(engine.plan(target, { version: 1, conversations: {} }).errors.includes('header 26.5814: 补丁标记不完整'));
        fs.writeFileSync(target.headerPath, patchedHeader);
        const host = fs.readFileSync(target.extensionJsPath, 'utf8').replace('action:"metadataSaved",metadata:i', 'action:"broken",metadata:null');
        const hostDecoy = 'function nestedHostDecoy(){try{n?.postMessage?.({action:"metadataSaved",metadata:i})}catch{}}';
        fs.writeFileSync(target.extensionJsPath, host.replace('function codexLocalGroupsSavePromptGroup(e,t,r,n,o){', 'function codexLocalGroupsSavePromptGroup(e,t,r,n,o){' + hostDecoy));
        assert.ok(engine.plan(target, { version: 1, conversations: {} }).errors.includes('extension metadata 26.5814: 补丁标记不完整'));
        fs.writeFileSync(target.extensionJsPath, host.replace('n?.postMessage?.({', '({action:"metadataSaved",metadata:i});n?.postMessage?.({'));
        assert.ok(engine.plan(target, { version: 1, conversations: {} }).errors.includes('extension metadata 26.5814: 补丁标记不完整'));
      },
    },
    {
      name: 'fails closed when the 26.5814 metadata host contracts drift',
      run() {
        const target = createTarget();
        configure265814Features(target);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        ...
        const drifts = [
          'if(e.action==="promptConversationTitle"||e.action==="promptConversationGroup"){codexLocalGroupsPromptConversation',
          'if(e.action==="promptNewGroup"){codexLocalGroupsPromptNewGroup',
          'else if(e.action==="setPendingGroup"||e.action==="newConversationInGroup")',
          'action:"metadataSaved"',
          'this.handleMessage(e,c)',
        ];
        for (const marker of drifts) {
          fs.writeFileSync(target.extensionJsPath, host.replace(marker, 'brokenMetadataContract'));
          const errors = engine.plan(target, { version: 1, conversations: {} }).errors;
          assert.ok(errors.includes('extension metadata 26.5814: 补丁标记不完整'), marker);
        }
      },
    },
    {
      name: 'rejects 26.5814 Host callbacks moved into later decoy functions',
      run() {
        const target = createTarget();
        configure265814Features(target);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        assert.deepStrictEqual(engine.apply(target, { version: 1, conversations: {} }).errors, []);
        const host = fs.readFileSync(target.extensionJsPath, 'utf8');
        const callbacks = ['action:"metadataSaved",metadata:i', 'action:"metadataSaved",metadata:s'];
        for (const callback of callbacks) {
          fs.writeFileSync(target.extensionJsPath, host.replace(callback, 'action:"broken",metadata:null') + `function laterHostDecoy(){return{${callback}}}`);
          assert.ok(engine.plan(target, { version: 1, conversations: {} }).errors.includes('extension metadata 26.5814: 补丁标记不完整'), callback);
        }
        for (const callback of ['if(codexLocalGroupsHandleWebviewMessage(n))return;let o=Q9(n)', 'if(codexLocalGroupsHandleWebviewMessage(c,e))return;this.handleMessage(e,c)']) {
          fs.writeFileSync(target.extensionJsPath, host.replace(callback, 'brokenHostCallback') + `function laterHostDecoy(){${callback}}`);
          assert.ok(engine.plan(target, { version: 1, conversations: {} }).errors.includes('extension metadata 26.5814: 补丁标记不完整'), callback);
        }
      },
    },
    {
      name: 'fails closed when the 26.5814 opened title scope drifts',
      run() {
        const target = createTarget();
        configure265814Features(target);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        ...
        for (const marker of drifts) {
          fs.writeFileSync(target.headerPath, header.replace(marker, 'brokenOpenedTitleContract'));
          assert.ok(engine.plan(target, { version: 1, conversations: {} }).errors.includes('header 26.5814: 补丁标记不完整'), marker);
        }
      },
    },
    {
      name: 'fails closed when the 26.5814 producer or native panel guard drifts',
      run() {
        const target = createTarget();
        configure265814Features(target);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        ...
        const drifts = ['js(e.agentThreadId)', 'conversationId:r', 'js(r)', 'conversationId:e', '&&!dt', '&&!hn', '&&!ht', '&&!pt'];
        for (const marker of drifts) {
          fs.writeFileSync(target.appMainPath, main.replace(marker, ''));
          assert.ok(engine.plan(target, { version: 1, conversations: {} }).errors.includes('Codex UI 26.5814: 补丁标记不完整'), marker);
        }
      },
    },
    {
      ...
      run() {
        ...
        fs.writeFileSync(target.appMainPath, main.replace('yn=(rt.length>0||It)&&!dt&&!hn&&!ht&&!pt', 'yn=!1') + decoy);
        assert.ok(engine.plan(target, { version: 1, conversations: {} }).errors.includes('Codex UI 26.5814: 补丁标记不完整'));
      },
    },
    {
      name: 'rejects broken 26.5814 composer contracts hidden by a nested function decoy',
      run() {
        const target = createTarget();
        configure265814Features(target);
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        ...
        const broken = main.replace('yn=(rt.length>0||It)&&!dt&&!hn&&!ht&&!pt', 'yn=!1').replace('function vWr(){', 'function vWr(){' + decoy);
        fs.writeFileSync(target.appMainPath, broken);
        assert.ok(engine.plan(target, { version: 1, conversations: {} }).errors.includes('Codex UI 26.5814: 补丁标记不完整'));
        const activeId = main.replace('activeConversationId:ae', 'brokenActiveConversationId:ae').replace('function vWr(){', 'function vWr(){function nestedActiveIdDecoy(){activeConversationId:ae}');
        fs.writeFileSync(target.appMainPath, activeId);
        assert.ok(engine.plan(target, { version: 1, conversations: {} }).errors.includes('Codex UI 26.5814: 补丁标记不完整'));
      },
    },
    {
      name: 'reports 26.5814 diagnostics for release anchor drift',
      run() {
        const target = createTarget();
        configure265814Features(target);
        fs.writeFileSync(target.extensionJsPath, fs.readFileSync(target.extensionJsPath, 'utf8').replace('timeoutMs:3e4})},3e4)', 'timeoutMs:3e4})},4e4)'));
        fs.writeFileSync(target.appMainPath, fs.readFileSync(target.appMainPath, 'utf8').replace('reasoningEffort:`xhigh`}],Zdn=', 'reasoningEffort:`xhigh`}],broken='));
        fs.writeFileSync(target.appServerManagerSignalsPath, fs.readFileSync(target.appServerManagerSignalsPath, 'utf8').replace('function dgn(){return pgn(`recent-conversations`)}', 'function broken(){return pgn(`recent-conversations`)}'));
        ...
        assert.ok(errors.some((error) => error.includes('26.5814') && error.includes('Webview 看门狗')));
        assert.ok(errors.some((error) => error.includes('Codex power 26.5814')));
        assert.ok(errors.some((error) => error.includes('26.5814 project history')));
        assert.ok(errors.every((error) => !error.includes('26.5810') && !error.includes('jP')));
      },
    },
    {
      name: 'fails closed on an unknown Codex 26.5814 build',
      run() {
        const target = createTarget();
        configure265814Features(target);
        target.version = '26.5814.99999';
        const engine = new CodexPatchEngine({ nodePath: resolveNodePath(), skipSyntaxCheck: true, safeMode: true });
        const plan = engine.plan(target, { version: 1, conversations: {} });
        assert.ok(plan.errors.includes('不支持的 Codex 26.5814 build：26.5814.99999'));
        assert.deepStrictEqual(plan.changes, []);
        assert.deepStrictEqual(engine.apply(target, { version: 1, conversations: {} }).changed, []);
      },
    },
    {
      name: 'writes nothing before rejecting an unknown 26.5814 build with an unsafe bundle backup',
      run() {
        const target = createTarget();
        configure265814Features(target);
        target.version = '26.5814.99999';
        ...
        fs.writeFileSync(path.join(backups, `${path.basename(target.headerPath)}.before-codex-local-groups-20260818000000000-1.bak`), clean);
...
context.codexLocalGroupsPromptGroup('abc','/project');
context.codexLocalGroupsStartConversationInGroup('/project','需求A');
context.codexLocalGroupsPromptNewGroup('/project');
console.log(JSON.stringify({dispatched:dispatched.filter(item=>item.message.action!=='getMetadata'),hostMessages}));`;
  const result = childProcess.spawnSync(resolveNodePath(), ['-'], { input: script, encoding: 'utf8' });
  assert.strictEqual(result.status, 0, result.stderr);
  return JSON.parse(result.stdout);
...
if (${options.toggleMore === true}) {
  ...
  if (${options.toggleMoreTwice === true}) {
    findButton(expandedRows, '展开更多').props.onClick({ preventDefault() {}, stopPropagation() {} });
    fullRows = context.codexRecentTaskProjectRows(filteredItems, ${JSON.stringify(activeId)}, () => {}, CodexRow);
  }
  findButton(fullRows || expandedRows, '收起到最近 5 条').props.onClick({ preventDefault() {}, stopPropagation() {} });
